...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function isCompetitiveCard(card) {
    // Heuristic for competitive cards - adjust these thresholds as needed
    const competitiveThreshold = 4.0; // EDHREC score threshold
    const highPriceThreshold = 10.00; // USD price threshold
    
    return (card.edhrec_rank && card.edhrec_rank < 100) || // Top 100 EDH cards
           (card.prices?.usd && parseFloat(card.prices.usd) > highPriceThreshold) ||
